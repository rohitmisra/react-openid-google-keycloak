...
import { GoogleOAuth, KeyCloakOAuth } from "./auth";
...
apiRouter.get("/auth-url/:provider", async ctx => {
  switch (ctx.params.provider) {
    ...
    case "keycloak": {
      const { login_hint, scope, redirect_uri, prompt, state } = ctx.query;
      const url = new KeyCloakOAuth({ redirect_uri, scope }).getAuthURL({
        login_hint,
        prompt,
        state
      });
...
apiRouter.get("/auth-from-code/:provider", async ctx => {
  ...
  if (ctx.params.provider === "keycloak") {
    const { code, redirect_uri, scope } = ctx.query;
    const KeyCloakOAuthInstance = new KeyCloakOAuth({ redirect_uri, scope });
    const { response, error } = await KeyCloakOAuthInstance.getTokenFromCode(code);
    if (response) {
      ctx.response.status = 200;
      const parsedResponse = KeyCloakOAuthInstance.parseResponse(
        response.body as string
      );
      const decodedResponse = KeyCloakOAuthInstance.parseJWTToken(
        parsedResponse && parsedResponse.id_token
      );
      if (decodedResponse) {
        const session = uuid();
